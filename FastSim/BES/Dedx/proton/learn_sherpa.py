...
class Net(torch.jit.ScriptModule):
    ...
    def __init__(self, N_hidden, N_ne, act):
        ...
        self.flayer = nn.Sequential( nn.Linear(3,N_ne),
                                      nn.ReLU()
                                    )
        l_list = []
...
if __name__ == '__main__':
    ...
    for trial in study:
        ...
        optimizer = optim.Adam(net.parameters(), lr=lr)## maybe Adam is not good here
        criterion = rel_mae_cost() ## seems resonable than none relative ?
        net.to(device)
        criterion.to(device)
    #####################################
        # Train model
        ...
        for epoch in range(epochs):
            total_cost = 0
            count = 0
            for ifile in [datafile]:
                print('h5 file=',ifile)
                h5 = h5py.File(ifile, 'r')
                df = h5['dataset'][0:max_evt,] if max_evt<h5['dataset'].shape[0] else h5['dataset'][:]
                h5.close()
                df = shuffle(df)
                df = df.astype('float32')
                df = np.ascontiguousarray(df)
                print('df shape=',df.shape)
                xb = np.ascontiguousarray(df[:,0:2]) #p, theta, dedx
