...
#sys.path.append('/hpcfs/juno/junogpu/fangwx/torch-two-sample/torch_two_sample')
sys.path.append('/hpcfs/juno/junogpu/fangwx/torch-two-sample/')
import pyximport; pyximport.install(build_dir="/hpcfs/juno/junogpu/fangwx/torch-two-sample/")
from torch_two_sample import statistics_diff
from torch.autograd import Variable
# try to get the lose for different part of dedx distribution, for example, left tail, main part, right tail
    ...
    def __init__(self, N_layer, N_ne, act):
        ...
        for i in range(self.layers):
            if i == 0: 
                ...
                #l_list.append(nn.ReLU( inplace=False ))
...
'''            
class Loss_KS(nn.Module):
    def __init__(self):
        super(Loss_KS, self).__init__()
...
if __name__ == '__main__':
    ...
    if cost_mode == 1:
        criterion = Loss_mse()
    print(criterion)
    #loss_fn = statistics_diff.SmoothKNNStatistic(1, 1, True, 1, compute_t_stat=True)    
    loss_fn = statistics_diff.SmoothKNNStatistic(batch_size, batch_size, True, 1, compute_t_stat=True)    
        ...
        for epoch in range(epochs):
            ...
            for i in range(df.shape[0]):
                ...
                if(i%10000==0): 
                    ...
                    logger.info('')
                #if i in used:continue ## check to see if remove this will help
                ...
                loss = loss_fn(Variable(train_y_t), outputs, alphas=[0.1])
                #print('loss=',loss)
                loss.backward()
                optimizer.step()
                loss_c = loss.cpu()
                ...
                '''
                outputs = net(train_x_t)
                outputs, _  = torch.sort(outputs  , dim=0, descending=False, out=None)#return a tuple (,)
                train_y_t,_ = torch.sort(train_y_t, dim=0, descending=False, out=None)
                p_left_tail = 0.1     
                outputs   = outputs  [0:int(p_left_tail*batch_size),]           
                train_y_t = train_y_t[0:int(p_left_tail*batch_size),]           
                optimizer.zero_grad()
                loss = criterion(outputs, train_y_t)
                loss.backward()
                optimizer.step()
