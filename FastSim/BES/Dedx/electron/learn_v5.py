...
# combined e, mu, p ,k ,pi. input dataset is arranged in gammabeta, beta, costheta, dedx_hit. The input 
# of NN can be (gammabeta,costheta,N) or (beta,costheta,N) 
# try different nets for different theta region, also do transfer learning like 
...
def load_data(data, max_n, index):
    print('load data:',data)
    d = h5py.File(data, 'r')
    #df = d['dataset'][:]
    max_evt = max_n if max_n < d['dataset'].shape[0] else d['dataset'].shape[0]
    df0 = d['dataset'][:max_evt,index:index+1]# gamma*beta or beta
    df1 = d['dataset'][:max_evt,2:4          ]# costheta, dedx
    df = np.concatenate((df0, df1), axis=1)
    d.close()
...
def get_parser():
    ...
    parser.add_argument('--transfer_ckpt_path', action='store', type=str,
                        help='transfer_ckpt_path ckpt file paths')
    ...
    parser.add_argument('--stage', action='store', type=str,
                        help='stage')
    ...
    parser.add_argument('--IndexFile', action='store', type=str,
                        help='IndexFile')
    parser.add_argument('--MinD', action='store', type=float, default=999,
                        help='min distance')

    parser.add_argument('--adam-beta', action='store', type=float, default=0.5,
                        help='Adam beta_1 parameter')
    parser.add_argument('--para_index', action='store', type=int, default=0,
                        help='first para index')
...
'''
class SNet(torch.jit.ScriptModule):
    __constants__ = ['fcs']
    def __init__(self, N_hid, N_ne, act, restore_path):
        super(SNet, self).__init__()
        self.b1 = Nett(N_hid+1, N_ne, act)
        self.b2 = Nett(N_hid+1, N_ne, act)
        self.b3 = Nett(N_hid+1, N_ne, act)
        self.b1.load_state_dict(torch.load(restore_path)) 
        self.b2.load_state_dict(torch.load(restore_path)) 
        self.b3.load_state_dict(torch.load(restore_path)) 
        l_list = [self.b1, self.b2, self.b3]
        self.fcs = nn.ModuleList(l_list)
    ...
    def forward(self, x):
        ...
        #print('x=',x.shape())
        if torch.mean(torch.acos(x[:,1])*180/math.pi) < 35 :
            y = self.b1(x)
            return y
        elif torch.mean(torch.acos(x[:,1])*180/math.pi) < 145 :
            y = self.b2(x)
            return y
        else :
            y = self.b3(x)
...
class SNet(torch.jit.ScriptModule):
    ...
    def __init__(self, N_hid, N_ne, act, restore_path):
        super(SNet, self).__init__()
        self.E1_p1 = Nett(N_hid+1, N_ne, act)
        self.E2_p1 = Nett(N_hid+1, N_ne, act)
        self.B1_p1 = Nett(N_hid+1, N_ne, act)
        self.E1_p2 = Nett(N_hid+1, N_ne, act)
        self.E2_p2 = Nett(N_hid+1, N_ne, act)
        self.B1_p2 = Nett(N_hid+1, N_ne, act)
        self.E1_p1.load_state_dict(torch.load(restore_path)) 
        self.E2_p1.load_state_dict(torch.load(restore_path)) 
        self.B1_p1.load_state_dict(torch.load(restore_path)) 
        self.E1_p2.load_state_dict(torch.load(restore_path)) 
        self.E2_p2.load_state_dict(torch.load(restore_path)) 
        self.B1_p2.load_state_dict(torch.load(restore_path)) 
        l_list = [self.E1_p1, self.E2_p1, self.B1_p1, self.E1_p2, self.E2_p2, self.B1_p2 ]
        self.fcs = nn.ModuleList(l_list)
...
if __name__ == '__main__':
    ...
    transfer_ckpt_path = parse_args.transfer_ckpt_path
    ...
    para_index = parse_args.para_index
    stage        = parse_args.stage
    if para_index == 0:
        print('use gamma*beta')
    elif para_index == 1:
        print('use beta')
    else:
        print('wrong index')
    ...
    #net = Net(N_hidden+1, N_units, act_mode)
    net = Nett(N_hidden+1, N_units, act_mode)
    if stage == 'transfer':
        net = SNet(N_hidden  , N_units, act_mode, transfer_ckpt_path)
    print(net)
    ...
    if Restore:
        ...
        #net.eval() #used for inference
    if doTraining:
        print('preparing data list for training')
        logger.info('')
        cost_list = []
        df = load_data(datafile, max_training_evt, para_index)
        ...
        if reLoadIndex==False:
            ...
            #faiss.write_index(index, IndexFile)# has error for GPU version index
        else:
    ...
    if doValid:
        ...
        cost_valid = None 
        count = 0
        df = load_data(validation_file, max_valid_evt, para_index)
        if stage == 'transfer': batch_size = 1 ## here should use 1 for SNet
    ...
    if doTest:
        ...
        logger.info('')
        df = load_data(test_file, produceEvent, para_index)
        tmp_pred = np.zeros((df.shape[0],1))
        for i in range(df.shape[0]):
            ...
            tmp_pred[i,0] = y_pred
        final_out = np.concatenate ((df[:, 0:3], tmp_pred), axis=-1)
