...
if __name__ == '__main__':
    ...
    for l in range(2):
        ...
        features.append(build_regression_v3(image=calorimeter[l], epsilon=0.001))
    ...
    #reg_ptheta  = tf.expand_dims(reg_ptheta, -1)
    #reg_pphi    = tf.expand_dims(reg_pphi  , -1)
    #reg_rtheta  = tf.expand_dims(reg_rtheta, -1)
    #reg_rphi    = tf.expand_dims(reg_rphi  , -1)
    #reg         = concatenate([reg_ptheta, reg_pphi, reg_rtheta, reg_rphi])
    #print('reg shape:',reg.shape)
    regression_outputs = [reg_ptheta, reg_pphi, reg_rtheta, reg_rphi]
    regression_losses = ['mae', 'mae','mae','mae']
    ...
    print('metrics_names:',regression.metrics_names)
    ...
    f_DataSet.close() 
    ######### test data #######
    d = h5py.File('/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/data/data_04211_batch0_N5000.h5', 'r')
    test_first = np.expand_dims(d['firstHitTimeByPMT'][:batch_size], -1)
    test_second = np.expand_dims(d['nPEByPMT'][:batch_size], -1)
    test_infoMuon = d['infoMuon'][:batch_size,:4]
    d.close()
    test_infoMuon[:,0]=(test_infoMuon[:,0])/math.pi
    test_infoMuon[:,1]=(test_infoMuon[:,1])/math.pi
    test_infoMuon[:,2]=(test_infoMuon[:,2])/math.pi
    test_infoMuon[:,3]=(test_infoMuon[:,3])/math.pi
    test_first, test_second, test_infoMuon = shuffle(test_first, test_second, test_infoMuon, random_state=0)
    ...
    for epoch in range(nb_epochs):
        ...
        for ib in range(len(Batch)):
            ...
            for index in range(ibatch):
                ...
                info_batch  = [ info_batch[:,0], info_batch[:,1], info_batch[:,2], info_batch[:,3] ]
                result1= regression.test_on_batch([image_batch_1, image_batch_2],info_batch)
                real_batch_loss = regression.train_on_batch(
                    [image_batch_1, image_batch_2],
                    info_batch
                )
                result2= regression.test_on_batch([image_batch_1, image_batch_2],info_batch)
                #print('index=',index,'batch loss=',real_batch_loss)
                #print('index=',index,'test1 batch loss=',result1)
                #print('index=',index,'test2 batch loss=',result2)
        ...
        #result1= regression.test_on_batch([test_first, test_second],[test_infoMuon[:,0], test_infoMuon[:,1], test_infoMuon[:,2], test_infoMuon[:,3]])
        logger.info('Epoch {:3d} test loss: {}'.format( epoch + 1, result1))
