...
if __name__ == '__main__':
    ...
    rate = tf.layers.dense(inputs=layer, units=1, activation=lambda x: tf.nn.elu(x) + 1)
    cost = Possion_cost(rate, y)
    ...
    with tf.Session() as sess:
        ...
        for itheta in range(theta.shape[1]):
            ...
            rate_pred, c_pred = sess.run([rate,cost],feed_dict={x:valid_x, y:n_pe[:,itheta:itheta+1]})
        ...
        for i in theta_list:
            ...
            rate_pred = sess.run(rate,feed_dict={x:i})
            s = np.random.poisson(rate_pred, produceEvent)
            s = s.reshape((-1,1))
            pred_n_pe[:,theta_list.index(i):theta_list.index(i)+1] = s
