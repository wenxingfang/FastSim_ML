...
###########################
## using Mix Destiny method exp + gauss
...
def exp_gaus_cost(y, Pred_y0, Pred_y):
    a      = Pred_y0
    LambDa = Pred_y[:0:1]
    sigma  = Pred_y[:1:2]
    mu     = Pred_y[:2:3]
    result = a*LambDa*tf.math.exp(-LambDa*y) + (1-a) * tf.math.exp(-tf.math.pow(x-mu,2)/(2*sigma*sigma))*1/(math.sqrt(2*math.pi)*sigma)
...
if __name__ == '__main__':
    ...
    Pred_y0 = tf.layers.dense(inputs=layer, units=1, activation = tf.math.sigmoid )
    Pred_y  = tf.layers.dense(inputs=layer, units=3)
    #cost = ks_cost(Pred_y, y)
    #cost = mse_cost(Pred_y, y)
    #cost = mae_cost(Pred_y, y)
    cost = exp_gaus_cost(y, Pred_y0, Pred_y)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for line in lines: 
                ...
                for ikey in list_key:
                    ...
                    inputs = np.concatenate ((r, theta), axis=-1)
        ...
        if savePb:
            ...
            # 写入序列化的 PB 文件
            with tf.gfile.FastGFile(pb_file_path+'/model_hit_time.pb', mode='wb') as f:
