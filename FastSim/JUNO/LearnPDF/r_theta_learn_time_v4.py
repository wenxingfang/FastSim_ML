...
def get_parser():
    ...
    parser.add_argument('--summary_path', action='store', type=str,
                        help='summary file paths')
    ...
    parser.add_argument('--saveCkpt', action='store', type=ast.literal_eval, default=False,
                        help='save ckpt file')
    parser.add_argument('--saveSummary', action='store', type=ast.literal_eval, default=False,
                        help='saveSummary file')
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        if saveCkpt:
            save_path = saver.save(sess, "%s/model.ckpt"%(ckpt_path))
        if saveSummary:
            print("Model summary saved  in path: %s" % summary_path)
            writer = tf.summary.FileWriter("%s"%summary_path, sess.graph)
            writer.close()
        if savePb:
