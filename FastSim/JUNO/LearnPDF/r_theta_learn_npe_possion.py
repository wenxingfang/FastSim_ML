...
if __name__ == '__main__':
    ...
    y = tf.placeholder(name='y',shape=(None,sample_size),dtype=tf.float32)
    layer = x
    for _ in range(3):
    ...
    #cost = Possion_cost(Pred_y, y)
    cost = Possion_cost_v1(Pred_y, y)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for line in lines: 
                ...
                n_batch = int(float(r_theta_npe.shape[0])/batch_size)
                for ib in range(n_batch):
                    value = r_theta_npe[ib*batch_size:(ib+1)*batch_size]
                    r     = value[:,0:1]/20 #r
                    theta = value[:,1:2]/180 #theta
                    train_x = np.concatenate ((r, theta), axis=-1)
                    n_sample = int(float(value.shape[1]-2)/sample_size)
                    for j in range(n_sample):
                        train_y = value  [:,2+j*sample_size:2+(j+1)*sample_size]
        ...
        for line in f_DataSet: 
            ...
            for ipmt in range(r_theta_npe.shape[0]):
                ...
                y_pred = np.random.poisson(y_pred, produceEvent)
