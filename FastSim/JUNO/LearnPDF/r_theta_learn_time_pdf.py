...
if __name__ == '__main__':
    ...
    for _ in range(3):
        ...
        layer = tf.layers.dense(inputs=layer, units=13, activation=tf.nn.tanh)
        ...
        for epoch in range(epochs):
            ...
            for line in lines: 
                ...
                for ikey in list_key:
                    ...
                    r     = np.array( [(float(ikey.split('_')[1])-10)/10])  #normalize
                    theta = np.array( [(float(ikey.split('_')[2])-90)/90])  #normalize
                    r     = np.reshape(r    , (1,1))
                    ...
                    #if n_batch < 5: n_batch = 5
                    if n_batch > 1 :
                        for ib in range(n_batch):
                            #index = np.random.randint(0,value.shape[0],(batch_size))## seems not good  
                            #train_x = inputs [index,]
                            #train_y = value  [index,]
