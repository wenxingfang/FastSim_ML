...
###########################
## batch size is now for different r,theta
...
def get_parser():
    ...
    parser.add_argument('--seq-size', action='store', type=int, default=2,
                        help='sequence size')
    parser.add_argument('--noise-size', action='store', type=int, default=1,
                        help='noise size')
...
if __name__ == '__main__':
    ...
    noise_size   = parse_args.noise_size
    ...
    assert produceEvent % seq_size == 0

    tf.reset_default_graph()
    x = tf.placeholder(name='x',shape=(None,2+noise_size),dtype=tf.float32)
    y = tf.placeholder(name='y',shape=(None,seq_size),dtype=tf.float32)
    layer = x
    '''
    for _ in range(3):
        #layer = tf.layers.dense(inputs=layer, units=12, activation=tf.nn.tanh)
        layer = tf.layers.dense(inputs=layer, units=100, activation=tf.nn.tanh)
    '''
    layer = tf.layers.dense(inputs=layer, units=100, activation=tf.nn.tanh)
    layer = tf.layers.dropout(layer,rate=0.2)
    layer = tf.layers.dense(inputs=layer, units=200, activation=tf.nn.tanh)
    layer = tf.layers.dropout(layer,rate=0.2)
    layer = tf.layers.dense(inputs=layer, units=400, activation=tf.nn.tanh)
    layer = tf.layers.dropout(layer,rate=0.2)
    layer = tf.layers.dense(inputs=layer, units=800, activation=tf.nn.tanh)
    layer = tf.layers.dropout(layer,rate=0.2)
    #Pred_y = tf.layers.dense(inputs=layer, units=1, activation=lambda x: tf.nn.elu(x) + 1)
    Pred_y = tf.layers.dense(inputs=layer, units=seq_size, activation=lambda x: tf.nn.relu(x), name = 'Pred')
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for line in lines: 
                ...
                #print('batchs=',batchs, ',r_theta_npe.shape[0]=',r_theta_npe.shape[0],',batch_size=',batch_size)
                #logger.info('')
                for ib in range(batchs):
                    value = r_theta_npe[ib*batch_size:(ib+1)*batch_size] 
                    n_seq = int(float(value.shape[1]-2)/seq_size)
                    #print('n seq=',n_seq, ',value.shape[1]=',value.shape[1],',seq_size=',seq_size)
                    #logger.info('')
                    for iq in range(n_seq):
                        label_y = value[:,2+iq*seq_size:2+(iq+1)*seq_size]
                        r     = value[:,0:1]/20   #normalize
                        theta = value[:,1:2]/180  #normalize
                        noise = np.random.uniform(-1, 1, (value.shape[0], noise_size)) if use_uniform else np.random.normal(0, 1, (value.shape[0], noise_size))
                        ...
                        _, c = sess.run([optimizer,cost], feed_dict={x:inputs, y:label_y})
                        total_cost += c
                        count = count + 1
                d.close()
            f_DataSet.close()
        ...
        for line in f_DataSet: 
            ...
            for ib in range(batchs):
                ...
                for iq in range(n_seq):
                    ...
                    noise = np.random.uniform(-1, 1, (value.shape[0], noise_size)) if use_uniform else np.random.normal(0, 1, (value.shape[0], noise_size))
                    inputs = np.concatenate ((r, theta, noise), axis=-1)
                    _, c = sess.run([Pred_y,cost], feed_dict={x:inputs, y:label_y})
                    cost_valid += c
                    count = count + 1
            d.close()
        f_DataSet.close()
        ...
        for line in f_DataSet: 
            ...
            for ipmt in range(r_theta_npe.shape[0]):
                ...
                Y_preds = 0
                First = True
                #print('int(produceEvent/seq_size)=',int(produceEvent/seq_size))
                for i in range(int(produceEvent/seq_size)):
                    noise = np.random.uniform(-1, 1, (1, noise_size)) if use_uniform else np.random.normal(0, 1, (1, noise_size))
                    inputs = np.concatenate ((r, theta, noise), axis=-1)
                    y_pred = sess.run(Pred_y,feed_dict={x:inputs})
                    y_pred = np.reshape(y_pred, (1, seq_size))
                ...
                Y_preds = np.reshape(Y_preds, (produceEvent))
                pred_nPEByPMT[ipmt,2:pred_nPEByPMT.shape[1]]=Y_preds
