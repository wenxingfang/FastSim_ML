...
if __name__ == '__main__':
    ...
    adam_beta = parse_args.adam_beta
    ...
    #learning_rate = 2e-5 # not good
    #optimizer = tf.train.AdamOptimizer(learning_rate).minimize(cost)
    optimizer = tf.train.AdamOptimizer(learning_rate, adam_beta).minimize(cost)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for ib in range(len(Batch)):
                ...
                for index in range(ibatch):
                    ...
                    list_theta = list(range(theta.shape[1]))
                    random.shuffle(list_theta)
                    #for itheta in range(theta.shape[1]):
                    for itheta in list_theta:
                        train_y = hit_time_batch[:,itheta:itheta+1]
                        train_y = del_empty(train_y)## for hit time clean
                        train_y = shuffle(train_y)
                        train_x = theta     [:,itheta:itheta+1]
                        train_x  =train_x.repeat(train_y.shape[0],axis=0)
        ...
        cost_valid = 0 
        for itheta in range(theta.shape[1]):
            valid_y = hit_time_batch[:,itheta:itheta+1]
            valid_y = del_empty(valid_y)## for hit time clean
            valid_x = theta     [:,itheta:itheta+1]
            valid_x  =valid_x.repeat(valid_y.shape[0],axis=0)
            noise = np.random.uniform(-1, 1, (valid_x.shape[0], 1))
            input_x = np.concatenate ((valid_x, noise), axis=-1)
            _, c_pred = sess.run([Pred_y,cost],feed_dict={x:input_x, y:valid_y})
        ...
        pred_hit_time = np.full((produceEvent, len(theta_list) ), 0 ,dtype=np.float32)#init
        for i in theta_list:
            ...
            pred_hit_time[:,theta_list.index(i):theta_list.index(i)+1] = y_pred*100 # back to unnormalized value
        hf = h5py.File(outFileName, 'w')
        hf.create_dataset('pred_hit_time', data=pred_hit_time)
