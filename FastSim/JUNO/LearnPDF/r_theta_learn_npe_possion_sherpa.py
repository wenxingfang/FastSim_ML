...
if __name__ == '__main__':
    ...
    parameters = [sherpa.Continuous('learning_rate', [1e-4, 1e-3]),
                  sherpa.Discrete('num_units', [10, 20]),
                  sherpa.Choice('activation', ['relu', 'tanh', 'sigmoid'])
                 ]
    algorithm = bayesian_optimization.GPyOpt(max_num_trials=10)
    ...
    for trial in study:
        ...
        #model.compile(loss=Possion_cost, optimizer=optimizer )
        # Train model
            ...
            for line in lines: 
                ...
                for ib in range(n_batch):
                    ...
                    for j in range(n_sample):
                        ...
                        c    = model.train_on_batch(train_x, train_y)
                        total_cost += c
                        count = count + 1
                d.close()
            f_DataSet.close()
