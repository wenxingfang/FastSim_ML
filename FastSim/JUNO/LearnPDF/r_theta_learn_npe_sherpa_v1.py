...
###########################
###########################
## one numpy for each r,theta pair, try with mean mae
...
def mae_cost_v1(label_y, pred_y):
...
def mae_cost_v2(label_y, pred_y):
    pred_y  = tf.sort(pred_y , axis=0,direction='ASCENDING',name=None)
    label_y = tf.sort(label_y, axis=0,direction='ASCENDING',name=None)
    abs_diff = tf.math.abs(pred_y - label_y) + 0.5
...
if __name__ == '__main__':
    ...
    #####################################
    logger.info('Preparing training data list')
    ...
    for line in lines: 
        ...
        try:
            ...
            for ikey in list_key:
                ...
                npeData_list.append('%s__%s'%(str(idata),ikey))
            d.close()
        except:
    ...
    for trial in study:
        ...
        #model.compile(loss=mae_cost_v1, optimizer=optimizer )
        model.compile(loss=mae_cost_v2, optimizer=optimizer )
        # Train model
