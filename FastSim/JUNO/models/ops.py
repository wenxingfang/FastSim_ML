...
def shift(ref, act):
    theta=act[0]*180/math.pi
    phi  =act[1]*180/math.pi
    theta = tf.cast(theta,dtype=tf.int32)
    phi   = tf.cast(phi  ,dtype=tf.int32)
    phi = tf.cond(tf.less(phi, 0), lambda:tf.add(phi,360), lambda:phi)
    #return [theta-ref[0], phi-ref[1]]
    return [ref[0]-theta, ref[1]-phi]

def roll(x, shift):
    print('roll:x.shape',x.shape)
    tmp = tf.roll(input=x, shift=shift, axis=[0,1])
    print('end roll')
    return tmp
    #return tf.roll(input=x, shift=shift, axis=[1,2])

def roll_fn(ele):
    print('ele[0] shape:',ele[0].shape)
    print('ele[1] shape:',ele[1].shape)
    p_theta = ele[1][2]
    p_phi   = ele[1][3]
    print('p_theta shape:',p_theta.shape)
    print('p_phi shape:',p_phi.shape)
    print('p_theta type:',type(p_theta))
    print('p_phi type:',type(p_phi))
    sh = shift((90, 180), (p_theta, p_phi))
    img = ele[0]
    tmp = roll(img, sh)
    print('end roll_fn')
    return tmp
...
def map_fn(ele):
    return tf.map_fn(fn=roll_fn, elems=ele, dtype=tf.float32,  parallel_iterations=None,  back_prop=True,  swap_memory=False,  infer_shape=True,  name=None)
...
def single_layer_energy(x):
    ...
    return K.reshape(K.sum(x, axis=list(range(1, len(shape)))), (-1, 1)) / 1000
...
def mini_batch_mean_1(x):
    x1 = x - x + K.mean(x, axis=0, keepdims=True)
    return x1
def mini_batch_std_1(x):
    x1 = x - x + K.std(x, axis=0, keepdims=True)
    return x1
...
class MyDense2D(Layer):
    ...
    def call(self, x):
        print('MyDense2D x',x.shape)
        print('MyDense2D kernel',self.kernel.shape)
        tmp = K.dot(x, self.kernel)
        print('MyDense2D K.dot(x, self.kernel)',tmp.shape)
