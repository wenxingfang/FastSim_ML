...
file: train_v6.py
##########################################################
Input image is 47486 array for time and nPE of PMT
###########################################################
"""
...
def load_data(data):
    ...
    '''
    print("first:",first.shape,", second:", second.shape,  ",info:", infoMC.shape, ",sizes:", sizes)
    first, second, infoMC = shuffle(first, second, infoMC)
    return first, second, infoMC
...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, sparsity_level, mini_batch_mean, mini_batch_mean_1 , mini_batch_std, mini_batch_std_1)

    from architectures import build_generator, build_discriminator, build_generator_v1, build_generator_v2,build_generator_v3,build_generator_v4, build_discriminator_v1, build_discriminator_v2,build_discriminator_v3, build_discriminator_v5, build_discriminator_v6, build_generator_v7, build_generator_v8, build_generator_v9
    ...
    calorimeter = Input(shape=(100,),dtype='float32')
    #calorimeter = Input(shape=(17612,),dtype='float32')
    #calorimeter = Input(shape=(47486,),dtype='float32')
    ...
    features=build_discriminator_v6(calorimeter, 0.001)
    ...
    npe_mean = Lambda(mini_batch_mean)(nPE)
    print('npe_mean:',npe_mean.shape)
    npe_std = Lambda(mini_batch_std)(nPE)
    print('npe_std:',npe_std.shape)
    npe_mean_1 = Lambda(mini_batch_mean_1)(calorimeter)
    print('npe_mean_1:',npe_mean_1.shape)
    npe_std_1 = Lambda(mini_batch_std_1)(calorimeter)
    print('npe_std_1:',npe_std_1.shape)



    p = concatenate([features, nPE, npe_mean, npe_std, npe_mean_1, npe_std_1])
    #p = features
    #fake = Dense(1, activation='sigmoid', name='fakereal_output')(p)
    fake = Dense(1, activation='sigmoid', kernel_initializer=RandomUniform(minval=-0.01, maxval=0.01, seed=None), name='fakereal_output')(p)
    ...
    out_size = 100#17612 #47486
    hidden_size = 1000#
    img_layer0 = build_generator_v9(generator_inputs, hidden_size, out_size)
    print('h2')
...
#############################
    ...
    for epoch in range(nb_epochs):
        ...
        epoch_disc_loss = []
        processed_batch = 0
        disc_outputs_real = 0.99*np.ones(batch_size)
        disc_outputs_real= bit_flip_v1(disc_outputs_real, flip_prob, 0.01)
        disc_outputs_fake = 0.01*np.ones(batch_size)
        ...
        for ib in range(len(Batch)):
            ...
            for index in range(ibatch):
                ...
                #image_batch_2 = second[index * batch_size:(index + 1) * batch_size, 0:17612]
                image_batch_2 = second[index * batch_size:(index + 1) * batch_size, 0:100]
                ...
                if index == (ibatch-1):
                    real_pred = discriminator.predict_on_batch(image_batch_2)
