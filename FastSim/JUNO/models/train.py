...
if __name__ == '__main__':
    ...
    from architectures import build_generator, build_discriminator, build_generator_v1
    ...
    def _load_data(particle, datafile):
        ...
        # convert to MeV
        energy = d['infoMuon'][:,5].reshape(-1, 1) * 1000 ## dummy , do not use it now
        ...
        #return first, second, y, energy, sizes
        return first, second, energy, sizes

    logger.debug('loading data from {} files'.format(nb_classes))

    first, second, energy, sizes = [
        np.concatenate(t) for t in [
            a for a in zip(*[_load_data(p, f) for p, f in s.items()])
        ]
    ]
    print("first:",first.shape,", second:", second.shape,  ",energy:", energy.shape, ",sizes:", sizes.shape)
    # TO-DO: check that all sizes match, so I could be taking any of them
    sizes = sizes[:4].tolist()
    ...
    #first, second, y, energy = shuffle(first, second, y, energy, random_state=0)
    first, second, energy = shuffle(first, second, energy, random_state=0)
    ...
    for l in range(2):
        ...
        energies.append(calculate_energy(calorimeter[l]))# not correct here!!
    ...
    layer0_T0 = Input(shape=(126, 226,1), name='l0_T0')
    layer0_T1 = Input(shape=(126, 226,1), name='l0_T1')
    layer1_T0 = Input(shape=(126, 226,1), name='l1_T0')
    layer1_T1 = Input(shape=(126, 226,1), name='l1_T1')
    ...
    img_layer0 = build_generator_v1(h, 126, 226, layer0_T0, layer0_T1)
    img_layer1 = build_generator_v1(h, 126, 226, layer1_T0, layer1_T1)
    print('h2')
    '''
    if not no_attn:
    ...
    d_temp.close()
    #print('Begin Multiply and Add')
    #Temps = [Input(shape=(126, 226,1)), Input(shape=(126, 226, 1)), Input(shape=(126, 226, 1)), Input(shape=(126, 226, 1))]
    #layer0_1 = multiply([generator_outputs[0], Temps[1]])
    #layer0_2 = add([layer0_1, Temps[0]])
    #layer1_1 = multiply([generator_outputs[1], Temps[3]])
    #layer1_2 = add([layer1_1, Temps[2]])
    #generator_outputs_new = [layer0_2, layer1_2]
    #generator_inputs = [latent]+Temps
    #print('End Multiply and Add')
    #############
...
###################################
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            #loss_weights = [np.ones(batch_size), 0 * np.ones(batch_size)] ## don't consider the energy effect now!!
            ...
            real_batch_loss = discriminator.train_on_batch(
                #[image_batch_1, image_batch_2, energy_batch],
