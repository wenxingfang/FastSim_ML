...
rec_dir = '/junofs/users/wxfang/FastSim/bes3/workarea/TestRelease/TestRelease-00-00-86/DedxTest/rec/output_off/'

rec_file = {}
files = os.listdir(rec_dir)
for i in files:
    if 'noDecay' not in i: continue
    if '_pi+_' not in i: continue
...
out_path = '/junofs/users/wxfang/FastSim/bes3/workarea/Mdc/DedxTrack/job_off/'
seed = 100
index = 0
for i in rec_file:
    str_particle = ''
    ...
    for line in lines:
        if 'NTupleSvc.Output' in line:
            ...
            line = line.replace('dummy.root','/junofs/users/wxfang/FastSim/bes3/workarea/Mdc/DedxTrack/output_off/%s'%ii)
            f_out.write(line)
