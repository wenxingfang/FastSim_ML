...
N_file = 100
N_event= 10000
template = '/junofs/users/wxfang/FastSim/bes3/workarea/TestRelease/TestRelease-00-00-86/DedxTest/sim/job_template_mu.txt'
f_in = open(template,'r')
lines = f_in.readlines()
out_path = '/junofs/users/wxfang/FastSim/bes3/workarea/TestRelease/TestRelease-00-00-86/DedxTest/sim/job_off/mu+/'
for i in range(N_file):
    f_out =open(out_path+'job_%d.txt'%i,'w')
    for line in lines:
        if 'RootCnvSvc.digiRootOutputFile' in line:
            line = line.replace('dummy','/junofs/users/wxfang/FastSim/bes3/workarea/TestRelease/TestRelease-00-00-86/DedxTest/sim/output_off/Single_mu+_off_%d.rtraw'%i)
            f_out.write(line)
