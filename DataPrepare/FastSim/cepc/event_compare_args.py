...
class Obj:
    ...
    def produce_cell_sum_energy(self):## 
        str1 = "" if self.is_real else "_gen"
        H_cell_sum_E = rt.TH1F('H_cell_sum_E_%s'%(str1)  , '', 150, 0, 150)
    ...
    def produce_ennergy_ratio(self):## 
        str1 = "" if self.is_real else "_gen"
        H_ratio_sum_E = rt.TH1F('H_ratio_sum_E_%s'%(str1)  , '', 100, 0.6, 1.4)
        hit_sum = np.sum(self.nB, axis=(1,2,3), keepdims=False)
        for i in range(self.nEvt):
...
if __name__ == '__main__':
    ...
    do_plot_v1(real_h_z_ps, fake_h_z_ps,'z_showershape',tag, str_particle)
    do_plot_v1(real_h_z_ps, fake_h_z_ps,'z_showershape_logy',tag, str_particle)
    real_h_y_ps = real.produce_y_sp()
    fake_h_y_ps = fake.produce_y_sp()
    do_plot_v1(real_h_y_ps, fake_h_y_ps,'y_showershape',tag, str_particle)
    do_plot_v1(real_h_y_ps, fake_h_y_ps,'y_showershape_logy',tag, str_particle)
    real_h_dep_ps = real.produce_dep_sp()
    fake_h_dep_ps = fake.produce_dep_sp()
    do_plot_v1(real_h_dep_ps, fake_h_dep_ps,'dep_showershape',tag, str_particle)
    do_plot_v1(real_h_dep_ps, fake_h_dep_ps,'dep_showershape_logy',tag, str_particle)
    ...
    do_plot_v1(real_h_cell_sum_E, fake_h_cell_sum_E,'cell_sum_energy',tag, str_particle)
    ...
    do_plot_v1(real_ratio, fake_ratio,'ratio_energy',tag, str_particle)
