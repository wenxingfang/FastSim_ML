...
if saveCellIdInfo:
    ...
    #    if entryNum >= N_max: break
        ...
        for i in range(0, len(tmp_Hit_x)):
            if tmp_cellID0[i] not in dicti_ID_x:
                dicti_ID_x[tmp_cellID0[i]]=tmp_Hit_x[i]
                dicti_ID_y[tmp_cellID0[i]]=tmp_Hit_y[i]
                dicti_ID_z[tmp_cellID0[i]]=tmp_Hit_z[i]
            #    dicti_ID_x[tmp_cellID0[i]]=[] 
            #    dicti_ID_y[tmp_cellID0[i]]=[] 
            #    dicti_ID_z[tmp_cellID0[i]]=[] 
            #    dicti_ID_x[tmp_cellID0[i]].append(tmp_Hit_x[i]) 
            #    dicti_ID_y[tmp_cellID0[i]].append(tmp_Hit_y[i]) 
            #    dicti_ID_z[tmp_cellID0[i]].append(tmp_Hit_z[i]) 
            #else:
            #    dicti_ID_x[tmp_cellID0[i]].append(tmp_Hit_x[i]) 
            #    dicti_ID_y[tmp_cellID0[i]].append(tmp_Hit_y[i]) 
            #    dicti_ID_z[tmp_cellID0[i]].append(tmp_Hit_z[i]) 
    #for iD in dicti_ID_x:
    #    if len(dicti_ID_x[iD])!=1:
    #        print('id=',iD,',x=',dicti_ID_x[iD])
    ''' 
    for iD in dicti_ID_x:
       dicti_ID_x[iD] = sum(dicti_ID_x[iD])/len(dicti_ID_x[iD])
       dicti_ID_y[iD] = sum(dicti_ID_y[iD])/len(dicti_ID_y[iD])
       dicti_ID_z[iD] = sum(dicti_ID_z[iD])/len(dicti_ID_z[iD])
    ''' 
    print('dicti_ID_x size=',len(dicti_ID_x))
    print('dicti_ID_y size=',len(dicti_ID_y))
    print('dicti_ID_z size=',len(dicti_ID_z))
    #print('dicti_ID_x',dicti_ID_x)
    '''
    ...
    for ID in dicti_ID_x:
        ...
        id_z = int(dicti_ID_z[ID]/10)
        if id_z not in table:
            print('exception z=', id_z)
            continue
        table[id_z][id_phi][ID]=[id_x, id_y, dicti_ID_z[ID]]
    json_str = json.dumps(table)
    with open('/junofs/users/wxfang/FastSim/cepc/ForReco/cell_ID.json', 'w') as json_file:
