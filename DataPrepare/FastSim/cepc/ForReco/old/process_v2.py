...
if saveCellIdInfo:
    ...
    for z in range(-z_max, z_max):
        ...
        for phi in range(360):
            dicts[z][phi] = np.full((1, 4), 0 ,dtype=np.float32)#init 
    ######### get cellID0 for hit ###############
    ...
    for entryNum in range(0, tree.GetEntries()):
        ...
        for i in range(0, len(tmp_Hit_x)):
            ...
            if tmp_cellID0[i] not in dicts[z][phi][:,0]:
                tmp = np.array([[tmp_cellID0[i],id_x,id_y,tmp_Hit_z[i]]])
                dicts[z][phi] = np.row_stack((dicts[z][phi],tmp))
    hf = h5py.File('/junofs/users/wxfang/FastSim/cepc/ForReco/cell_ID.h5', 'w')
    N_tot =0
    for z in dicts:
        for phi in dicts[z]:
            #dicts[z][phi]=np.delete(dicts[z][phi], [0], axis=0)
            N_tot = N_tot + dicts[z][phi].shape[0]-1
            hf.create_dataset('Hit_info_%d_%d'%(z,phi)   , data=dicts[z][phi])
    hf.close()
...
if saveCellIdInfo:
    ...
    #    for phi in range(360):
    #        dicts[z][phi] = np.full((1, 4), 0 ,dtype=np.float32)#init 
    Hit_info = np.full((1, 6), 0 ,dtype=np.float32)#init 
    ######### get cellID0 for hit ###############
    filePath = '/junofs/users/wxfang/FastSim/cepc/ForReco/meta/Dg_cellID_all.root'
    treeName='evt'
    chain =rt.TChain(treeName)
    ...
    for entryNum in range(0, tree.GetEntries()):
        ...
        for i in range(0, len(tmp_Hit_x)):
            ...
            id_z = int(tmp_Hit_z[i]/10)
            if abs(id_z) > z_max: continue
            if tmp_cellID0[i] not in Hit_info[:,0]:
                tmp = np.array([[tmp_cellID0[i], id_z, id_phi, id_x, id_y, tmp_Hit_z[i]]])
                Hit_info = np.row_stack((Hit_info,tmp))
    Hit_info=np.delete(Hit_info, [0], axis=0)
    hf = h5py.File('/junofs/users/wxfang/FastSim/cepc/ForReco/Hit_info_1000.h5', 'w')
    hf.create_dataset('Hit_info', data=Hit_info)
    hf.close()
    '''
    8*153*2*235*29=16683120
    '''
    print('dicti_ID size=',Hit_info.shape[0])
