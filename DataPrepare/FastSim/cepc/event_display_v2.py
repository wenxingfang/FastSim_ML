...
def mc_info(particle, mom, dtheta, dphi, Z):
    ...
    info.SetTextFont (   42 )
    info.AddText("%s (mom=%.1f GeV, #theta=%.1f, #phi=%.1f, Z=%.1f cm)"%(particle, mom, dtheta, dphi, Z/10))
    return info
...
plot_path=""
if show_real:
    hf = h5py.File('/hpcfs/juno/junogpu/fangwx/FastSim/CEPC/data/Dg_10_40_em_0.h5', 'r')
    ...
    hf = h5py.File('/hpcfs/juno/junogpu/fangwx/FastSim/CEPC/generator/Gen1007.h5', 'r')
    nB = hf['Barrel_Hit']
    nB = np.squeeze(nB)
    n3 = hf['MC_info']
...
for event in event_list:
    if n3 is not None:
        ...
        print ('event=%d, Mom=%f, dtheta=%f, dphi=%f, Z=%f'%(event, n3[event,0], n3[event,1], n3[event,2], n3[event,3]))
