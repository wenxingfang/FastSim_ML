...
def root2hdf5 (Max_event, nRows, nCols, tree, start_event, out_name, id_index_theta_phi):
    default_time = 0
    default_nPE =  0
    default_E   =  0
    ...
    energyPMT         = np.full((Max_event, nRows, nCols), default_E   , dtype=np.float32)#init 
    ...
    for ie in range(start_event, start_event+Max_event):
        ...
        for i in tmp_dict:
            if i not in id_index_theta_phi: continue
            i_theta   = id_index_theta_phi[i][0]
            i_phi     = id_index_theta_phi[i][1]
            firstHitTimeByPMT[entryNum, i_theta, i_phi] = tmp_dict[i][0]
            nPEByPMT         [entryNum, i_theta, i_phi] = tmp_dict[i][1]
            energyPMT        [entryNum, i_theta, i_phi] = tmp_dict[i][2]
...
if __name__ == '__main__':
    ...
    for i in range(batch):
        ...
        root2hdf5 (Max_event, nRows, nCols, tree, start, out_name, ID_index_theta_phi)
        start = start + Max_event
        if i == batch-1:
            out_name = outFileName.replace('.h5','_batch%d_N%d.h5'%(i+1,totalEntries%Max_event))
            root2hdf5 (totalEntries%Max_event, nRows, nCols, tree, start, out_name, ID_index_theta_phi)
