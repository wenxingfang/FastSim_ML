...
# npe will be saved in one dataframe, hit time will be saved in different dataframe according to the r, theta
def get_parser():
...
def root2hdf5 (Max_event, tree, start_event, out_name, id_dict, s_r):
    ...
    nPEByPMT          = np.full((len(id_list), Max_event+2), default_nPE , dtype=np.float32)#init 
    ...
    for ie in range(start_event, start_event+Max_event):
        ...
        for i in range(0, nPhotons):
            ...
            h_time = hitTime[i]
            ls_dict_htime[id_dict[ID]].append(h_time)
        ...
        for i in id_list:
            ind = id_list.index(i)
            if i in tmp_dict:
                nPEByPMT[ind,entryNum+2] = tmp_dict[i]
            else:
                nPEByPMT[ind,entryNum+2] = 0
    for i in id_list:
        ind = id_list.index(i)
        nPEByPMT[ind,0] = s_r 
        nPEByPMT[ind,1] = id_dict[i]

    hf = h5py.File(out_name, 'w')
    hf.create_dataset('nPEByPMT', data=nPEByPMT)
    s_r = round(s_r,3) # keep three digi
    for i in ls_dict_htime:
