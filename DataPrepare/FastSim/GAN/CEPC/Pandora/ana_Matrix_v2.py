...
class Obj:
    def __init__(self, name, fileName, max_evt):
        ...
        self.h_rec_E          = rt.TH1F('%s_h_rec_E'        %(str_ext) ,'',150,0,150)
        self.h_rec_E_res      = rt.TH1F('%s_h_rec_E_res'    %(str_ext) ,'',100,E_res_low, E_res_high)
        self.h_rec_theta_res  = rt.TH1F('%s_h_rec_theta_res'%(str_ext) ,'',40, Theta_res_low, Theta_res_high)
        self.h_rec_phi_res    = rt.TH1F('%s_h_rec_phi_res'  %(str_ext) ,'',40, Phi_res_low  , Phi_res_high)
    ...
    def fill(self):
        ...
        for entryNum in range(0, max_evt):
            ...
            tmp_pz    = getattr(tree, "m_pReco_pz")
            '''
            tmp_hits_x    = getattr(tree, "m_hits_x")
            tmp_hits_y    = getattr(tree, "m_hits_y")
            tmp_hits_z    = getattr(tree, "m_hits_z")
            tmp_hits_E    = getattr(tree, "m_hits_E")
            for i in range(len(tmp_hits_x)):
                self.h_hit_x_y.Fill(tmp_hits_x[i], tmp_hits_y[i], tmp_hits_E[i])
                self.h_hit_x_z.Fill(tmp_hits_x[i], tmp_hits_z[i], tmp_hits_E[i])
                self.h_hit_y_z.Fill(tmp_hits_y[i], tmp_hits_z[i], tmp_hits_E[i])
                self.h_hit_E.Fill(tmp_hits_E[i])
            '''
            ...
            #led_reco = findLed0(tmp_px, tmp_py, tmp_pz)
...
if __name__ == '__main__':
    ...
    ''' 
    #do_plot(obj_pan.h_rec_theta     ,"Pan_reco_theta_%s"%tag        ,{'X':"#theta_{pfo}^{%s} degree"%str_led_pa          ,'Y':'Events'}, '' )
    #do_plot(obj_pan.h_rec_phi       ,"Pan_reco_phi_%s"%tag          ,{'X':"#phi_{pfo}^{%s} degree"%str_led_pa            ,'Y':'Events'}, '' )
    #do_plot(obj_pan.h_rec_px        ,"Pan_reco_Px_%s"%tag           ,{'X':"Px_{pfo}^{%s} GeV/c"%str_led_pa               ,'Y':'Events'}, '' )
    #do_plot(obj_pan.h_rec_py        ,"Pan_reco_Py_%s"%tag           ,{'X':"Py_{pfo}^{%s} GeV/c"%str_led_pa               ,'Y':'Events'}, '' )
    #do_plot(obj_pan.h_rec_pz        ,"Pan_reco_Pz_%s"%tag           ,{'X':"Pz_{pfo}^{%s} GeV/c"%str_led_pa               ,'Y':'Events'}, '' )
    do_plot(obj_pan.h_rec_E         ,"Pan_reco_E_%s"%tag            ,{'X':"E_{pfo}^{%s} GeV"%str_led_pa                  ,'Y':'Events'}, '' )
    do_plot(obj_pan.h_rec_E_res     ,"Pan_reco_E_res_%s"%tag        ,{'X':"(E_{pfo}^{%s}-E_{mc}^{%s})/E_{mc}^{%s}"%(str_led_pa,str_led_pa,str_led_pa)  ,'Y':'Events'},'' )
    do_plot(obj_pan.h_rec_theta_res ,"Pan_reco_theta_res_%s"%tag    ,{'X':"#theta_{pfo}^{%s}-#theta_{mc}^{%s}"%(str_led_pa,str_led_pa)             ,'Y':'Events'},'' )
    do_plot(obj_pan.h_rec_phi_res   ,"Pan_reco_phi_res_%s"%tag      ,{'X':"#phi_{pfo}^{%s}-#phi_{mc}^{%s}"%(str_led_pa,str_led_pa)                 ,'Y':'Events'},'' )
    
    do_eff_plot(obj_pan.h_mc_theta         , obj_pan.h_mc_theta_all     , 'Pan_reco_eff_theta_%s'%tag, {'X':"#theta_{mc}^{%s} (degree)"%(str_led_pa)  ,'Y':'Efficiency'}, '10<#theta_{mc}<170, E_{mc} > 0.1 GeV')
    do_eff_plot(obj_pan.h_reco_mc_E_num    , obj_pan.h_reco_mc_E_den    , 'Pan_reco_eff_E_%s'%tag    , {'X':"E_{mc}^{%s} (GeV)"%(str_led_pa)       ,'Y':'Efficiency'}   , '10<#theta_{mc}<170, E_{mc} > 0.1 GeV')
    do_eff_plot(obj_pan.h_reco_mc_phi_num  , obj_pan.h_reco_mc_phi_den  , 'Pan_reco_eff_phi_%s'%tag  , {'X':"#phi_{mc}^{%s} (degree)"%(str_led_pa)    ,'Y':'Efficiency'}, '10<#theta_{mc}<170, E_{mc} > 0.1 GeV')
    f_reco_eff_out = rt.TFile(root_out, 'UPDATE')
    f_reco_eff_out.cd()
    obj_pan.h_rec_E_res.Write("%s_%s"%(obj_pan.h_rec_E_res.GetName(), tag))
    obj_pan.h_rec_theta_res.Write("%s_%s"%(obj_pan.h_rec_theta_res.GetName(), tag))
    obj_pan.h_rec_phi_res.Write("%s_%s"%(obj_pan.h_rec_phi_res.GetName(), tag))
    f_reco_eff_out.Close()
    ''' 
    do_plot   (obj_pan.h_hit_E   ,"Hits_E"   ,{'X':"Hit E (GeV)",'Y':'Entries'}   ,'' )
    do_plot_2D(obj_pan.h_hit_x_y ,'h_hit_x_y', {'X':"Hit x (mm)",'Y':'Hit y (mm)'}, '')
    do_plot_2D(obj_pan.h_hit_x_z ,'h_hit_x_z', {'X':"Hit x (mm)",'Y':'Hit z (mm)'}, '')
    do_plot_2D(obj_pan.h_hit_y_z ,'h_hit_y_z', {'X':"Hit y (mm)",'Y':'Hit z (mm)'}, '')
    ''' 
