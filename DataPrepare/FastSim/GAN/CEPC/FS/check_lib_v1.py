...
if __name__ == '__main__':
    ...
    h_mc_mom      = F_in.Get('h_mc_mom'     )	
    h_mc_theta    = F_in.Get('h_mc_theta'   )	
    h_mc_phi      = F_in.Get('h_mc_phi'     )	
    h_mc_v_x      = F_in.Get('h_mc_v_x'     )	
    h_mc_v_y      = F_in.Get('h_mc_v_y'     )	
    h_mc_v_z      = F_in.Get('h_mc_v_z'     )	
    h_E_nFS      = F_in.Get('E_nFS'     )	
    h_E_tag0     = F_in.Get('E_tag0'    )	
    h_E_tag1     = F_in.Get('E_tag1'    )	
    h_E_tag2     = F_in.Get('E_tag2'    )	
    h_phi_nFS      = F_in.Get('phi_nFS'     )	
    h_phi_tag0     = F_in.Get('phi_tag0'    )	
    h_phi_tag1     = F_in.Get('phi_tag1'    )	
    h_phi_tag2     = F_in.Get('phi_tag2'    )	
    ...
    plot_gr(h_mc_mom ,'h_mc_mom' ,['Mom_{mc} (MeV)' ,'Events']  ,1)
    plot_gr(h_mc_theta ,'h_mc_theta' ,['#theta_{mc} (degree)' ,'Events']  ,1)
    plot_gr(h_mc_phi   ,'h_mc_phi'   ,['#phi_{mc} (degree)' ,'Events']  ,1)
    plot_gr(h_mc_v_x   ,'h_mc_v_x'   ,['#X_{mc} (mm)' ,'Events']  ,1)
    plot_gr(h_mc_v_y   ,'h_mc_v_y'   ,['#Y_{mc} (mm)' ,'Events']  ,1)
    plot_gr(h_mc_v_z   ,'h_mc_v_z'   ,['#Z_{mc} (mm)' ,'Events']  ,1)
    ...
    plot_gr(h_phi_nFS ,'h_phi_nFS' ,['x (mm)' ,'N FS']  ,1)
    plot_gr(h_phi_tag0,'h_phi_tag0',['x (mm)' ,'N tag0'],1)
    plot_gr(h_phi_tag1,'h_phi_tag1',['x (mm)' ,'N tag1'],1)
    plot_gr(h_phi_tag2,'h_phi_tag2',['x (mm)' ,'N tag2'],1)
