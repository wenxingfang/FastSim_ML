...
class Obj:
    ...
    def fill(self):
        ...
        theta_list = list(range(0,180,1))
        h_theta_dict = {}
        for i in range(len(theta_list)-1):
            h_theta_dict["%d_%d"%(theta_list[i], theta_list[i+1])] = rt.TH1F('%s_theta_%d_%d'%(self.name, theta_list[i], theta_list[i+1]), '', 50, 0, 0.05)
        ...
        #for entryNum in lines:
            ...
            for i in h_theta_dict:
                if float(i.split('_')[0]) < tmp_theta and tmp_theta < float(i.split('_')[1]):
                    h_theta_dict[i].Fill(1000*En/tmp_Mom)
        return h_theta_dict
...
obj_G4        = Obj('G4','/cefs/higgs/wxfang/cepc/FS/check_sim_G4/*_to.root' ,100000)
obj_FS        = Obj('FS','/cefs/higgs/wxfang/cepc/FS/check_sim_FS/*_to.root' ,100000)

scale_to_G4 = True


fs_h_theta_dict = obj_FS.fill()
g4_h_theta_dict = obj_G4.fill()
for i in g4_h_theta_dict:
    if g4_h_theta_dict[i].GetSumOfWeights()<100:continue
    do_plot_h2(g4_h_theta_dict[i]   ,fs_h_theta_dict[i]   ,'h_res_theta_%s'%str(i)    ,{'X':'#sum Hit_{E}/mc_{E}'     ,'Y':'Events'}, '' , scale_to_G4)
    print('fs=%f,g4=%f, ratio=%f'%(fs_h_theta_dict[i].GetMean(), g4_h_theta_dict[i].GetMean(), fs_h_theta_dict[i].GetMean()/g4_h_theta_dict[i].GetMean() ))
