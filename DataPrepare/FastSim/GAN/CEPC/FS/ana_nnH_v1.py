...
def do_plot_h2(h_real, h_fake, out_name, title, plot_label, doFit):
    canvas=rt.TCanvas("%s"%(out_name),"",800,800)
    canvas.cd()
    canvas.SetTopMargin(0.13)
...
#    h_real.Scale(1/h_real.GetSumOfWeights())
    ...
    if doFit: 
        ...
        err2   = result.ParError (2)
        status = result.Status()
        str_para = 'G4:mean=%.3f#pm%.3f, #sigma=%.3f#pm%.3f'%(par1, err1, par2, err2)
        func = h_real.GetFunction("f1")
        func.Draw('SAME')
    label_r=rt.TLatex(0.15, 0.92 , "%s"%(str(str_para)))
    label_r.SetTextSize(0.03)
    label_r.SetNDC()
    label_r.Draw()
    ############ 
    str_para = ''
    ...
    if doFit:
        ...
        err2   = result.ParError (2)
        status = result.Status()
        str_para = 'FS:mean=%.3f#pm%.3f, #sigma=%.3f#pm%.3f'%(par1, err1, par2, err2)
        func = h_fake.GetFunction("f1")
...
class Obj:
    ...
    def fill(self):
        ...
        for entryNum in range(0, tree.GetEntries()):
            ...
            if self.doAll==True:
                ...
                if led_gamma !=-1 and sub_gamma != -1:
                    ...
                    isBarrel0 = True if ( (P4_rec_led_g.Theta()*180/math.pi) > 50 and (P4_rec_led_g.Theta()*180/math.pi) < 140 ) else False
                    isBarrel1 = True if ( (P4_rec_sub_g.Theta()*180/math.pi) > 50 and (P4_rec_sub_g.Theta()*180/math.pi) < 140 ) else False
                    if isBarrel0 == False or isBarrel1 == False: continue
        ...
        print('eff=%f'%(self.h_rec_led_g_E.GetSumOfWeights()/totalEntries))
                     
############ BEGIN ##############

plot_path = './event_plots/'

off_obj_pan        = Obj('OffPfo','/cefs/higgs/wxfang/cepc/FS/off_nnHaa_digi/Digi_sim_*.root')
off_obj_pan.setPID(p1=22, p2=22)

#fs_obj_pan        = Obj('FSPfo','/cefs/higgs/wxfang/cepc/FS/nnHaa_apply_digi//Digi_sim*.root')
#fs_obj_pan        = Obj('FSPfo','/cefs/higgs/wxfang/cepc/FS/nnHaa_ETunning_digi/Digi_sim*.root')
fs_obj_pan        = Obj('FSPfo','/cefs/higgs/wxfang/cepc/FS/EnScale_apply_v1_nnHaa_digi/Digi_sim*.root')
fs_obj_pan.setPID(p1=22, p2=22)
str_led_pa = "#gamma"
str_sub_pa = "#gamma"
...
do_plot_h2(off_obj_pan.h_rec_led_g_E   , fs_obj_pan.h_rec_led_g_E    ,"reco_led_g_E"       ,{'X':"E_{pfo}^{%s} GeV"%str_led_pa                           ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_led_g_px  , fs_obj_pan.h_rec_led_g_px   ,"reco_led_g_Px"      ,{'X':"Px_{pfo}^{%s} GeV/c"%str_led_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_led_g_py  , fs_obj_pan.h_rec_led_g_py   ,"reco_led_g_Py"      ,{'X':"Py_{pfo}^{%s} GeV/c"%str_led_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_led_g_pz  , fs_obj_pan.h_rec_led_g_pz   ,"reco_led_g_Pz"      ,{'X':"Pz_{pfo}^{%s} GeV/c"%str_led_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_sub_g_E   , fs_obj_pan.h_rec_sub_g_E    ,"reco_sub_g_E"       ,{'X':"E_{pfo}^{%s} GeV"%str_sub_pa                           ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_sub_g_px  , fs_obj_pan.h_rec_sub_g_px   ,"reco_sub_g_Px"      ,{'X':"Px_{pfo}^{%s} GeV/c"%str_sub_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_sub_g_py  , fs_obj_pan.h_rec_sub_g_py   ,"reco_sub_g_Py"      ,{'X':"Py_{pfo}^{%s} GeV/c"%str_sub_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_sub_g_pz  , fs_obj_pan.h_rec_sub_g_pz   ,"reco_sub_g_Pz"      ,{'X':"Pz_{pfo}^{%s} GeV/c"%str_sub_pa                        ,'Y':'Events'}, '' , False )
do_plot_h2(off_obj_pan.h_rec_m_gg      , fs_obj_pan.h_rec_m_gg       ,"reco_gg_Mass"       ,{'X':"M_{pfo}^{%s%s} GeV/c^{2}"%(str_led_pa,str_sub_pa)      ,'Y':'Events'}, '' , True  )
do_plot_h2(off_obj_pan.h_rec_m_gg_reco , fs_obj_pan.h_rec_m_gg_reco  ,"reco_gg_reco_Mass"  ,{'X':"M_{pfo}^{%s%s reco} GeV/c^{2}"%(str_led_pa,str_sub_pa) ,'Y':'Events'}, '' , False )



print('saved led E hist with events=',off_obj_pan.h_rec_led_g_E.GetSumOfWeights())
print('saved led E hist with events=',fs_obj_pan.h_rec_led_g_E.GetSumOfWeights())
#print('done')
'''
