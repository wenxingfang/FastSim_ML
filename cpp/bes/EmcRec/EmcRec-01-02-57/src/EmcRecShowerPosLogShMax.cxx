//log       emcShowerPosLogShMax.cxx
// ShowerMax    Logarithmic weighted position attribute reconstruction
//
//    Created 
//
...
//  cout<<"EmcRecShowerPosLogShMax::Position Begin"<<endl; 
  ...
  double len55=(log(e55/0.0145)+0.5)*1.86;

  if(Para.PositionMode2()=="all") {
    double etot=aShower.getEAll();
    double ltot=(log(etot/0.0145)+0.5)*1.86;
...
//              theDistance=(seedPoint.mag()+ltot)/cos(seedPoint.angle(hitPoint));
    ...
    double l3x3=(log(e3x3/0.0145)+0.5)*1.86;
    Identifier   seedID = aShower.getShowerId();
    HepPoint3D seedPoint(iGeoSvc->GetCFrontCenter(seedID));
    //Rs is the distance from orgin to showerCentroid, unit cm
//    HepDouble Rs=seedPoint.mag();
...
//          HepDouble Ang=seedPoint.angle(hitPoint);
          ...
          double theDistance;
            if(cit->second.getCellId()==seedID) {
              theDistance=(seedPoint.mag()+l3x3);
            } else {
              theDistance=(seedPoint.mag()+l3x3)/cos(seedPoint.angle(hitPoint));
            }
      if(w>0){
        wsum+=w;
        pos=iGeoSvc->GetCFrontCenter(cit->second.getCellId());
    ...
    double l5x5=(log(e5x5/0.0145)+0.5)*1.86;
    Identifier   seedID = aShower.getShowerId();
    HepPoint3D seedPoint(iGeoSvc->GetCFrontCenter(seedID));
    //Rs is the distance from orgin to showerCentroid, unit cm
//    HepDouble Rs=seedPoint.mag();
...
//          HepDouble Ang=seedPoint.angle(hitPoint);
          ...
          double theDistance;
            if(cit->second.getCellId()==seedID) {
              theDistance=(seedPoint.mag()+l5x5);
            } else {
              theDistance=(seedPoint.mag()+l5x5)/cos(seedPoint.angle(hitPoint));
            }
...
//  aShower.setPosition(pos);
  ...
  if(module==1) {   //barrel
    ...
    for(int i=0;i<5;i++){
      ...
      if(e55>1.0)     	                 parTheta[i] = Para.BarrShLogThetaPara(theModule,i);
      else if(e55<=1.0 && e55>0.5)       parTheta[i] = Para.BarrShLogThetaPara(theModule+22,i);
      else if(e55<=0.5)                  parTheta[i] = Para.BarrShLogThetaPara(theModule+44,i);

      if(e55>1.0)                        parPhi[i] = Para.BarrShLogPhiPara(0,i);
      else if(e55<=1.0 && e55>0.5)       parPhi[i] = Para.BarrShLogPhiPara(1,i);
      else if(e55<=0.5)                  parPhi[i] = Para.BarrShLogPhiPara(2,i);
    }
