...
:BesSensitiveDetector(name)
{
    ...
    //string ele_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_em.pt"; 
    string ele_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_e-_twoSP.pt"; 
    char* cstr_e = new char[ele_bp_file.size() + 1];
    strcpy(cstr_e, ele_bp_file.c_str());
    m_e_DedxPred  = new NNPred(cstr_e);
    string pp_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_p+.pt"; 
    char* cstr_pp = new char[pp_bp_file.size() + 1];
    strcpy(cstr_pp, pp_bp_file.c_str());
    m_pp_DedxPred  = new NNPred(cstr_pp);
    string kp_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_k+.pt"; 
    char* cstr_kp = new char[kp_bp_file.size() + 1];
    strcpy(cstr_kp, kp_bp_file.c_str());
    m_kp_DedxPred  = new NNPred(cstr_kp);
    string pip_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_pi+.pt"; 
    char* cstr_pip = new char[pip_bp_file.size() + 1];
    strcpy(cstr_pip, pip_bp_file.c_str());
    m_pip_DedxPred  = new NNPred(cstr_pip);
    string mup_bp_file = "/junofs/users/wxfang/FastSim/bes3/workarea/Simulation/BOOST/MdcSim/MdcSim-00-00-73/src/NN/model_mu+.pt"; 
    char* cstr_mup = new char[mup_bp_file.size() + 1];
    strcpy(cstr_mup, mup_bp_file.c_str());
    m_mup_DedxPred  = new NNPred(cstr_mup);
    //std::cout << "Hi 1" << std::endl;
    
    float* tmp_p_dEdx_hit = new float[100];
    t_proton = new TChain("n103");
    t_proton->Add("/junofs/users/wxfang/FastSim/GAN/BES/Dedx/raw_data/proton/proton.root");
    t_proton->SetBranchAddress("ptrk"    , &m_p_ptrk);
    t_proton->SetBranchAddress("charge"  , &m_p_charge);
    t_proton->SetBranchAddress("costheta", &m_p_costheta);
    t_proton->SetBranchAddress("ndedxhit", &m_p_ndedxhit);
    t_proton->SetBranchAddress("dEdx_hit", tmp_p_dEdx_hit);
    m_p_dEdx_hit = tmp_p_dEdx_hit;
    std::cout<<"total entries for proton is "<<t_proton->GetEntries()<<std::endl;
    for(unsigned i=0; i<10; i++)
    {
        t_proton->GetEntry(i);
        std::cout<<"i="<<i<<",ptrk="<<m_p_ptrk<<",charge="<<m_p_charge<<",costheta="<<m_p_costheta<<",ndedxhit="<<m_p_ndedxhit<<std::endl;
        if(m_p_ndedxhit>=100)continue;
        for(unsigned j=0; j<m_p_ndedxhit; j++)
        {
            std::cout<<"j="<<j<<",dEdx_hit="<<tmp_p_dEdx_hit[j]<<std::endl;
        }
    }
...
G4bool BesMdcSD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
  ...
  const G4ParticleDefinition* def = gTrack->GetDefinition();
  ...
  if(m_G4Svc->GetMdcDedxFlag()==1){
     ...
     ///--- NN-- //////
     float mom = sqrt( gTrack->GetMomentum()[0]*gTrack->GetMomentum()[0] + gTrack->GetMomentum()[1]*gTrack->GetMomentum()[1] + gTrack->GetMomentum()[2]*gTrack->GetMomentum()[2] )/1000; //to GeV 
     float costheta = cos(theta); 
     //std::cout<<"mom="<<mom<<",costheta="<<costheta<<std::endl;
     G4double pred = 0;
     if(abs(pdgCode)==11)
     {
         pred = m_e_DedxPred->get(1, mom/2.0, costheta, 228, 638);
         //if(mom<0.3) pred = eCount;
     }
     else if(abs(pdgCode)==2212)//proton
     {
         pred = m_pp_DedxPred->get(2, mom, costheta, 1000, 1000);
     }
     else if(abs(pdgCode)==321)//kaon
     {
         pred = m_kp_DedxPred->get(3, mom, costheta, 900, 750);
     }
     else if(abs(pdgCode)==211)//pion
     {
         pred = m_pip_DedxPred->get(4, mom, costheta, 600, 550);
     }
     else if(abs(pdgCode)==13)//muon
     {
         pred = m_mup_DedxPred->get(5, mom/2.0, costheta, 600, 550);
         pred = eCount;
     }
     else//not defined
     {
     ...
     edep=pred;
     //std::cout<<"layerId="<<layerId<<",cellId="<<cellId<<",mom="<<mom<<",costheta="<<costheta<<",pred="<<pred<<",off="<<eCount<<",pdg="<<pdgCode<<std::endl;
     if(edep<=0) return false; 
  ...
  if (hitPointer[layerId][cellId] == -1) {
      ...
      //std::cout<<"insert:"<<"edep="<<edep<<",pdg="<<pdgCode<<std::endl;
  }
  else
  {
