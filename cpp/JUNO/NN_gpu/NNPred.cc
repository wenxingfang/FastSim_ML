...
//NNPred::NNPred()
{
  ...
  std::cout<<"cuda::is_available="<<torch::cuda::is_available() << "ok\n";
...
tmp_vec.push_back(0.5);
*/
//at::Tensor input_ = torch::from_blob(in_array, {3,3}, options).to(torch::Device("cuda:0"));
at::Tensor input_ = torch::from_blob(tmp_vec.data(), {N_size,3}, options).to(torch::Device("cuda:0"));
//t1 = clock();
...
else if(type==3) 
{
    ...
    module_3.to(torch::Device("cuda:0"));
...
else 
{
    ...
    module_4.to(torch::Device("cuda:0"));
