...
MuonFastSimVoxel::UserSteppingAction(const G4Step* step) {
    ...
    /*
    if(m_track_id==0) m_track_id = track->GetTrackID();
    else if(m_track_id == track->GetTrackID() ){ 
                                                   if(m_pos0.x()==0 && m_pos0.y()==0 && m_pos0.z()==0) m_pos0 = pos_v1 ;
                                                   else {
                                                         G4ThreeVector dpos = pos_v1-m_pos0; m_pos0 = pos_v1; 
                                                         std::cout<<"Dpos="<<dpos.mag()<<" mm, edep="<<edep<<" MeV"<<std::endl;
                                                        }
                                               }
    else if(m_track_id != track->GetTrackID() ){ m_track_id = track->GetTrackID(); m_pos0 = pos_v1; }
    */ 
    ...
    //t0 = clock();
    //std::cout << "Hi 2 dt="<<(t0-t1) / (double) CLOCKS_PER_SEC<<",t0="<<t0<<",t1="<<t1<<std::endl;
    ...
    //t1 = clock();
    //std::cout << "Hi 3 dt="<<(t1-t0) / (double) CLOCKS_PER_SEC<<",t0="<<t0<<",t1="<<t1<<std::endl;
...
MuonFastSimVoxel::generate_hits_v3(float r , const std::vector<float>& vec_theta, const std::vector<int>& vec_id, float frac, double start_time)
{
    ...
    /*
    std::vector<float> tmp_theta;
    tmp_theta.push_back(0.11);
    tmp_theta.push_back(0.12);
    tmp_theta.push_back(0.13);
    std::vector<float>* tmp_pred  = m_NNpred_npe_high->get(2, 16, tmp_theta, 1) ; 
    */
    ...
    //t1 = clock();
    //std::cout << "Hi 11 dt="<<(t1-t0) / (double) CLOCKS_PER_SEC<<",t0="<<t0<<",t1="<<t1<<std::endl;
    ...
    //t0 = clock();
    //std::cout << "Hi 22 dt="<<(t0-t1) / (double) CLOCKS_PER_SEC<<",t0="<<t0<<",t1="<<t1<<std::endl;
